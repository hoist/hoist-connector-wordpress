...
var ConnectorRequiresAuthorizationError = _errors2.default.create({ name: 'ConnectorRequiresAuthorizationError' });

var WordpressPoller = function () {
  function WordpressPoller(context) {
    _classCallCheck(this, WordpressPoller);

    this._logger = _logger2.default.child({ cls: this.constructor.name, subscription: context.subscription._id, application: context.application._id });
  ...
  _createClass(WordpressPoller, [{
    ...
    value: function poll() {
      ...
      return this.assertCanPoll().then(function () {
        ...
        _this._context.subscription.delayTill((0, _moment2.default)().add(1, 'minutes').toDate());
      }).catch(function (err) {
    ...
    value: function generateUrlForEndpoint(endpoint) {
      ...
      return Promise.resolve().then(function () {
        ...
        if (filterDate) {
          query += 'after=' + (0, _moment2.default)(filterDate).toISOString();
        }
        ...
        if (documents.length < 1) {
          ...
          return Promise.all(documents.map(function (document) {
            ...
            return documents.map(function (d) {
              return (0, _moment2.default)(d.date);
            }).sort().pop().toDate();
